...
class Application():
    ...
    def count_word(self, file_name):
        ...
        # 定义字典 决定了统计的格式
        dic = {}
        with open(source_file_name, 'r', encoding=('UTF-8')) as fr:  # 读入已经去除停用词的文件
            data = jieba.cut(fr.read().strip())
        data = dict(Counter(data))
        with open(res_file_name, 'w', encoding=('UTF-8')) as fw:  # 读入存储wordcount的文件路径
            for k, v in data.items():
                if k != '\n' and k != '' and k != ' ' and k != '	' and k != ':':
                    fw.write('%s:%d\n' % (k, v))
                    dic[k] = v
