...
class Application():
    ...
    def eventMarge(self):
        if self.stat:  # 判断当前看控件的状态
            ...
            #启动线程
            try:
                _thread.start_new_thread(self.merge_key,(4,))
            except:
                self.listbox.insert(END,"线程启动失败!")
            self.stat = True
        self.btnPart["state"] = "active"
...
# -----------------------------------------------网络爬虫----------------------------------------------------
    ...
    def count_word(self,file_num):
        ...
        # 文件的名称
        # file_num =3
        source_file_name = path + str(file_num) + "_C.txt"
        res_file_name = res_path + str(file_num) + "_CC.txt"
        # 统计关键词的个数
        # 计算机文件的行数
    ...
    def merge_key(self,value):
        ...
        dic1 = []
        dic2 = []
        dic1 = self.count_word(num1)
        dic2 = self.count_word(num2)
        ...
        resultFloat = result * 100
        resultStr = "文档"+num1+"和"+ num2+"的相似度是:"+str(resultFloat)+"%"
        self.listbox.insert(END,resultStr)
        return TRUE
