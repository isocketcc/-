...
class Application():
    ...
    listbox.grid(row=1, column=0, columnspan=6, rowspan=90, sticky=S + N + W + E)

    def __init__(self, width=860, height=420):
        self.w = width
        self.h = height
        self.stat = True
        self.staIco = None
    ...
    def GridBtn(self):
        ...
        self.btnDelWord = Button(self.root, text="删除停用词", command=self.eventDelWord, width=19, height=3)
        ...
        self.btnClust.grid(row = 0,column = 4)
    ...
    def read_file_CD(self, value):
        ...
        while num < 500:
            ...
            # 读取内容
            source_str = source_fo.readline().split()
            ...
            while source_str:
                ...
                result_fo.write((result_str).encode('UTF-8'))
                result_str = ""
            num = num + 1  # 处理文档数量加1
    ...
    def count_word(self, file_name):
        ...
        res_file_name = res_path + temp[len(temp)-1]
        # 统计关键词的个数
        # 计算机文件的行数
        ...
        while source_line:
            ...
            # 字典的插入与赋值
            for word in words:
                if word != " " and word != "\n" and word != "\t" and word != "	" and word != "" and word in table:
                    num = table[word]
                    table[word] = num + 1
    ...
    #---------------------------------------------------------统计----------------------------------------------
    def add_paper(self):
        if os.path.exists("resChese\\res\\统计\\all.txt"):
            print("文档统计完毕!")
        else:
            # 默认情况下处理500个文档
            begNum = 1
            endNum = 499
            paperNum = begNum
            result_paper = "resChese\\res\\统计\\all.txt"
            result_paper_fw = open(result_paper, 'w', encoding='UTF-8')
            while paperNum < endNum:
                source_paper = "resChese\\res\\分词化\\" + str(paperNum) + "_C.txt"
    ...
    def set_cluster(self,value):
        ...
        corpus = []
        # 读取预料 一行预料为一个文档
        for line in open('resChese\\res\\统计\\all.txt', 'rb').readlines():
        ...
        resName = "resChese\\res\\统计\\BHTfidf_Result.txt"
        ...
        print(weight)
        fw = open("resChese\\res\\统计\\wieght.txt", 'w', encoding='UTF-8')
        fw.write(str(weight))
